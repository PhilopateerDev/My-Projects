...
from sklearn.ensemble import RandomForestRegressor  # تم تغيير المكتبة هنا
from sklearn.preprocessing import StandardScaler
from sklearn.metrics import r2_score

# --- إعدادات الصفحة ---
st.set_page_config(page_title="House Price Predictor")
st.title("🏠 House Price Prediction (Random Forest)")
...
# بنحول أسماء المناطق لأرقام عشان الموديل يقدر يحسبها
...
# الترتيب في X حسب الصورة: Area ثم Rooms ثم Bathrooms ثم Location
X = df.drop('price', axis=1)
y = df['price']

# --- تقسيم البيانات لتدريب واختبار ---
# بنستخدم 80% للتدريب و 20% للاختبار و random_state=42
...
# بنعرف موديل الغابة العشوائية وبندربه
...
# --- الرسم البياني لدقة التوقع ---
st.subheader("Prediction Accuracy Chart")
fig, ax = plt.subplots()
ax.scatter(y_test, y_pred, color='green', alpha=0.5) # غيرنا اللون للأخضر تمييزاً للغابة
ax.plot([y_test.min(), y_test.max()], [y_test.min(), y_test.max()], 'r--', lw=2)
ax.set_xlabel('Actual Price')
ax.set_ylabel('Predicted Price')
st.pyplot(fig)

# --- مدخلات المستخدم بنفس ترتيب أعمدة الملف ---
st.divider()
st.subheader("Enter House Details for Prediction:")

# الترتيب: Area -> Rooms -> Bathrooms -> Location
...
if st.button("Predict Price Now"):
    # ترتيب المدخلات في مصفوفة يطابق X_train
    user_input = np.array([[area, rooms, bathrooms, location_val]])
    
    # تحويل المقياس (Scaling)
